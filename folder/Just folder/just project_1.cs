using System;
	...
	public class Program
    {
		...
		public static void FillingConstantvalues(int[,] m) //заполнение QR-кода неизменными значениями
		{
			...
			//заполянем "красные клетки" для маски
			for (int i = 0;i<4;i++)
			{
				m[i+9,0] = 4;
				m[i+9,3] = 4;
    	...
    	{	
			...
			do
			{ 
				Console.WriteLine("Введите кодируемый текст");
				Console.WriteLine();
    			//s = Console.ReadLine();
				...
				{
					Console.WriteLine("Превышено допустимое колличество памяти");
					return;
				}
			}	
			while(s.Length > 152);
			...
			FillMatrixFromString(s);//итоговое создание QR-кода c последующим заполнением его из стороки
		}
	}